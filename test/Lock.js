  ...
  describe("Withdrawals", function () {
    ...
    describe("Events", function () {
      it("Should emit an event on withdrawals", async function () {
        ...
        expect(event.args[0]).to.equal(lockedAmount);
        expect(event.args[1]).to.be.an.instanceOf(ethers.BigNumber);
        expect(event.args[1].gte(ethers.BigNumber.from(unlockTime))).to.be.true;
      });
    });
